...
def simulate_markov_chain_diploid(K: int,
                                 q: np.ndarray,
                                 r: float,
                                 steps: int,
                                 d_values,
                                 ...
                                 rng=None):
    """
    Simulate diploid genotypes X_t in {0,1,2} from the linkage HMM by assuming
    two independent haplotypes (a,b) with the same ancestry Markov chain model.

      Z^{(a)}_0 ~ q,  Z^{(b)}_0 ~ q   (independent)
      Z^{(h)}_t | Z^{(h)}_{t-1}=j  ~  Q(d_t, r, q)  for h in {a,b}
      A_t | Z^{(a)}_t=k  ~ Bernoulli(p[k,t])
      B_t | Z^{(b)}_t=l  ~ Bernoulli(p[l,t])
      X_t = A_t + B_t

    Parameters
    ----------
    K, q, r, steps, d_values, p : as in simulate_markov_chain

    Returns
    -------
    X : np.ndarray (steps,), entries in {0,1,2}
    Q_last : np.ndarray (K,K) transition matrix at last step (or None if steps<2)
    """
    if rng is None:
        rng = np.random.default_rng()
    ...
    # Initial hidden states for the two haplotypes
    z_a = rng.choice(np.arange(K), p=q)
    z_b = rng.choice(np.arange(K), p=q)

    X = np.zeros(steps, dtype=int)

    # genotype at t=0
    a0 = int(rng.binomial(1, p[z_a, 0]))
    b0 = int(rng.binomial(1, p[z_b, 0]))
    X[0] = a0 + b0

    Q_last = None
    for t in range(1, steps):
        Q = create_transition_matrix(K, q, float(r), float(d_values[t]))
        z_a = rng.choice(np.arange(K), p=Q[z_a])
        z_b = rng.choice(np.arange(K), p=Q[z_b])

        a = int(rng.binomial(1, p[z_a, t]))
        b = int(rng.binomial(1, p[z_b, t]))
        X[t] = a + b
        Q_last = Q
