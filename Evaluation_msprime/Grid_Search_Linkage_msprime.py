...
def forward_log_likelihood_diploid(X, q, p, d, r):
    """
    Diploid extension of forward_log_likelihood.

    Observation X[t] is a diploid genotype in {0,1,2} produced by two independent
    haplotypes with hidden ancestries (Z^a_t, Z^b_t), each following the same
    linkage transition model.

    Hidden state at t is the pair (i,j) with i,j in {0..K-1}.

    Emission (given p_i=p[i,t], p_j=p[j,t]):
      P(X=0) = (1-p_i)(1-p_j)
      P(X=2) = p_i p_j
      P(X=1) = p_i(1-p_j) + (1-p_i)p_j

    Returns log P(X | q,p,d,r).
    """
    X = np.asarray(X, dtype=int)
    q = _normalize_prob_vec(q)
    p = np.asarray(p, dtype=float)
    ...
    if np.any((X < 0) | (X > 2)):
        raise ValueError("Diploid X must be in {0,1,2}.")

    # Initial joint distribution: outer(q,q)
    alpha = np.outer(q, q)  # (K,K)

    # emission at t, vectorized
    def emit_matrix(t):
        pc = p[:, t]
        p_i = pc[:, None]
        p_j = pc[None, :]
        if X[t] == 0:
            return (1.0 - p_i) * (1.0 - p_j)
        if X[t] == 2:
            return p_i * p_j
        # X[t] == 1
        return p_i * (1.0 - p_j) + (1.0 - p_i) * p_j
    ...
    for t in range(1, M):
        ...
        # Transition matrix for a haplotype
        Q = stay * np.eye(K) + switch * q[None, :]  # rows sum to 1

        # predicted joint distribution: Q^T @ alpha @ Q
        predicted = (Q.T @ alpha) @ Q

        alpha = predicted * emit_matrix(t)
        scale[t] = alpha.sum()
        if scale[t] <= 0:
            return -np.inf
        alpha /= scale[t]
...
# ---- Faster override: exploit the special transition structure to compute Q^T alpha Q in O(K^2) ----

def forward_log_likelihood_diploid(X, q, p, d, r):
    """Fast diploid forward log-likelihood.

    Same model as the earlier forward_log_likelihood_diploid, but computes the
    prediction step using the closed-form expansion of
        predicted = Q^T @ alpha @ Q
    for
        Q = stay*I + (1-stay)*1*q^T.

    This reduces the per-step cost from O(K^3) to O(K^2).
    """
    X = np.asarray(X, dtype=int)
    q = _normalize_prob_vec(q)
    p = np.asarray(p, dtype=float)
    ...
    for t in range(1, M):
        ...
        # closed-form predicted = Q^T alpha Q
        # where Q = stay I + switch * 1 q^T
        rowsum = alpha.sum(axis=1)   # (K,)
        colsum = alpha.sum(axis=0)   # (K,)
        total = 1.0  # alpha is scaled to sum to 1

        predicted = (stay * stay) * alpha
        predicted += stay * switch * (np.outer(q, colsum) + np.outer(rowsum, q))
        predicted += (switch * switch) * total * np.outer(q, q)
