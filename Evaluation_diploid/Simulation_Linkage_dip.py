...
def simulate_markov_chain_diploid(K: int,
                                 ...
                                 p: np.ndarray,
                                 rng=None):
    """Simulate diploid genotype observations X_t in {0,1,2}.

    Diploid extension where an individual carries two independent ancestry paths
    (one per haplotype) with the same linkage transition.

    Emissions (Hardy–Weinberg conditional on ancestries):
      A_t | Z_t^(a)=k ~ Bernoulli(p[k,t])
      B_t | Z_t^(b)=l ~ Bernoulli(p[l,t])
      X_t = A_t + B_t  in {0,1,2}
    """
    if rng is None:
        rng = np.random.default_rng()
    ...
    # Initial hidden states (two haplotypes)
    z1 = rng.choice(np.arange(K), p=q)
    z2 = rng.choice(np.arange(K), p=q)

    X = np.zeros(steps, dtype=int)
    X[0] = int(rng.binomial(1, p[z1, 0]) + rng.binomial(1, p[z2, 0]))

    Q_last = None
    for t in range(1, steps):
        Q = create_transition_matrix(K, q, r, d_values[t])
        z1 = rng.choice(np.arange(K), p=Q[z1])
        z2 = rng.choice(np.arange(K), p=Q[z2])
        X[t] = int(rng.binomial(1, p[z1, t]) + rng.binomial(1, p[z2, t]))
        Q_last = Q
